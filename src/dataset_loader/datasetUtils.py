...
def imgAug(inputImage, crop=False, flip=False, gaussianBlur=False, channelInvert=False, brightness=False, hueSat=False, rotate=False):
    augList = []
    if crop:
...
def imageRandomAugmentation(
    ...
    if imAug:
        if np.random.rand()<imAugPr:
            gaussian, brightness, hueSat, channelInvert, rotate, crop, flip = np.random.choice(a=[False, True], size=7, p=[0.5, 0.5])
            inputImage = imgAug(inputImage=inputImage, crop=crop, flip=flip, rotate=rotate,
    ...
    if padding:
        ...
        inputImage = cv2.copyMakeBorder(
            ...
            left=colPadding, right=colPadding, borderType=cv2.BORDER_CONSTANT, value=[128., 128., 128.])
    ...
    inputImage = cv2.warpAffine(inputImage, affineMat, (imageCol, imageRow), None, cv2.INTER_CUBIC, cv2.BORDER_CONSTANT, (128., 128., 128.))
