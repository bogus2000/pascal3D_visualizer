...
class nolbo_test(object):
    ...
    def getPred(self,
        ...
        idx_start, idx_end = 1, 1+4
        # bbox2D_list = enc_output_selected[..., idx_start:idx_end]
        idx_start, idx_end = idx_end, idx_end + 3
        bbox3D_list = enc_output_selected[..., idx_start:idx_end]
        idx_start, idx_end = idx_end, idx_end + 16
    ...
    def _encOutPartitioning(self):
        ...
        # self._bbox2D = tf.sigmoid(tf.transpose(tf.stack(self._bbox2D), [1, 2, 3, 0, 4]))
        self._bbox2D = tf.transpose(tf.stack(self._bbox2D), [1, 2, 3, 0, 4])
        self._bbox2D = tf.concat(
            [tf.exp(self._bbox2D[..., :2]) * self._anchor_boxes,
             tf.sigmoid(self._bbox2D[..., 2:])], axis=-1)
        self._bbox3D = tf.transpose(tf.stack(self._bbox3D), [1,2,3,0,4])
