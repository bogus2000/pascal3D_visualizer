...
class classifier(object):
    def __init__(self, backbone_style=None, model_backbone=None, name=None,
        ...
        with self._strategy.scope():
            ...
            # self._optimizer = tf.keras.optimizers.Nadam(learning_rate=self._learning_rate)
            # self._optimizer = tf.keras.optimizers.SGD(learning_rate=self._learning_rate, momentum=0.9, nesterov=True, decay=0.0001)
    ...
    def _buildModel(self):
        print('build Models...')
        self.model_backbone = darknet.Darknet19(name=self._name+'_backbone',
                                                activation=self._b_act)
        self.model_backbone.summary()
        # ================================= set model head
        self.model_head = darknet.head2D(name=self._name+'_head2D',
                                         input_shape=[None,None,1024],
                                         output_dim=self._output_dim,
                                         filter_num_list=self._last_fnl,
                                         filter_size_list=self._last_fsl,
                                         last_pooling=self._last_pooling,
                                         activation=None)
    # @tf.function
    def fit(self, inputs):
        ...
        with tf.GradientTape() as tape:
            output_backbone = self.model_backbone(input_images, training=True)[0]
            ...
            total_loss = pred_loss #+ 1e-4*reg_loss
            # total_loss += tf.reduce_sum(self.model_backbone.losses + self.model_head.losses)
